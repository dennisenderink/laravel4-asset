...
class Asset_Container
{
	...
	{
		$type = $source ? $this->defineType($source) : null;

		if ($type) {
			return $this->$type($name, $source, $dependencies, $attributes);
		} else {
			// check is enrolled
			$asset = $this->findEnrolled($name);

			return $this->register($asset['type'], $name);
		}
	}

	/**
	 * Defines type from source
	 * @param string $source
	 * @return string
	 */
	private function defineType($source)
	{
	 ...
	 * @return Asset_Container
	 */
	public function style($name, $source = null, $dependencies = array(), $attributes = array())
	{
	...
	public function enrollStyle($name, $source, $dependencies = array(), $attributes = array())
	{
		if (!array_key_exists('media', $attributes)) {
			$attributes['media'] = 'all';
		}

		$this->enrollNow('style', $name, $source, $dependencies, $attributes);
	 ...
	 * @return Asset_Container
	 */
	public function script($name, $source = null, $dependencies = array(), $attributes = array())
	{
	...
	public function enrollScript($name, $source, $dependencies = array(), $attributes = array())
	{
		$this->enrollNow('script', $name, $source, $dependencies, $attributes);
	...
	/**
	 * Enroll an asset to be used later on
	 *
	 * @param string $type
	 * @param string $name
	 * @param array $dependencies
	 ...
	 * @return Asset_Container
	 */
	public function enroll($name, $source, $dependencies = array(), $attributes = array())
	{
		$type = $this->defineType($source);

		return $this->{'enroll' . ucfirst($type)}($name, $source, $dependencies, $attributes);
	}
	 ...
	 * @return void
	 */
	protected function register($type, $name, $source = null, $dependencies = array(), $attributes = array())
	{
		if (!$source) {
			// check if enrolled
			if ($this->isEnrolled($name, $type)) {
				// extract values
				extract($this->enrolled[$type][$name]);
 			}
		}
	...
	/**
	 * Adds asset to list of enrolled assets
	 *
	 * @param string $type
	 * @param string $name
	 * @param string $source
	 ...
	 * @return Asset_Container
	 */
	protected function enrollNow($type, $name, $source, $dependencies, $attributes)
	{
		$dependencies = (array) $dependencies;
		$attributes = (array) $attributes;

		$this->enrolled[$type][$name] = compact('source', 'dependencies', 'attributes');

		return $this;
	}

	private function findEnrolled($name, $type = null)
	{
		$enrolled = false;
		if (!$type) {
		...
		if ($enrolled) {
			return array(
				'type' => $type,
				'data' => $this->enrolled[$type][$name],
			);
		} else {
			\Log::warning('No enrolled asset with name: "' . $name . '" found.');
		}
	}

	private function isEnrolled($name, $type)
	{
		return isset($this->enrolled[$type]) && isset($this->enrolled[$type][$name]);
	}
